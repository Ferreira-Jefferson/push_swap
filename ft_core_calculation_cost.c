...
size_t	ft_index_to_insert_in_b_reverse(t_deque *list_a, int value)
{
	...
	while (current)
	{
		if ((value < min || value > max) && current->value == min)
			return (index);
		if (current->value <= value && current->next && value < current->next->value)
			return (index + 1);
		current = current->next;
		index++;
	}
...
void	ft_calculate_cost_b_reverse(t_deque *list_a, t_deque *list_b)
{
	...
	while (current)
	{
		index = ft_index_to_insert_in_b_reverse(list_a, current->value);
...
int	ft_calculate_cost_b_return(t_deque *list, int index)
{
	...
	while (head)
	{
		head->cost_b = ft_calculate_cost_b_return(list, current_index);
		head = head->next;
		current_index++;
	}
	return (0);
